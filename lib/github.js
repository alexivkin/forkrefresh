...
const octokit = new octokitlib.Octokit()
...
module.exports = {
  ...
  getAllBranches : async (owner, repo) => {
    return await octokit.paginate(octokit.repos.getBranches,{owner, repo});
  },

  getLastCommits: async (owner,repo, branch,per_page) => {
    // reverse chronological order
    return (await octokit.repos.listCommits({owner, repo, sha: branch, per_page})).data //, sha, path, author, since, until, per_page, page})
  },

  getAllCommits: async (owner,repo, branch) => {
    // https://octokit.github.io/rest.js/#api-Repos-listCommits
    return await octokit.paginate(octokit.repos.listCommits,{owner, repo, sha: branch, per_page:100}) //, sha, path, author, since, until, per_page, page})
  },

  fastForward: async (owner,repo, branch, sha) => {
    return await octokit.git.updateRef({owner, repo, ref:'heads/'+branch, sha, force:true}) // force:true overrides all changes in the source
  },
  ...
  getTreeWithAPI : async (reponame,branch) => {
    ...
    const b = await octokit.paginate(octokit.repos.getBranch,{owner:creds.username, repo:reponame, branch:branch}); // gotta paginate for 'commit' to come through
    // console.log(b.commit)
    // paginate otherwise the tree.tree is async
    const result = await octokit.paginate(octokit.git.getTree,{owner:creds.username, repo:reponame, tree_sha:b.commit.commit.tree.sha, recursive:1});
    if (result.truncated){
